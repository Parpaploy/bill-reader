...
import { parseBillWords } from "./lib/parseBill";
  ...
  async function handleScan() {
    ...
    try {
      const res = await fetch("/api/ocr", {
        ...
        body: JSON.stringify({ imageBase64: stripDataUrlPrefix(imagePreview) }),
      });
      ...
      setRows(parseBillWords(data.words));
      setStatus("done");
    } catch (err) {
      setErrorMessage(
    ...
    <div className="min-h-screen bg-paper">
      <header className="border-b border-brass/30 px-6 py-8 md:px-12">
        ...
        <p className="font-body text-ink-soft text-sm mt-2 max-w-xl">
          ...
          ขับเคลื่อนด้วย Google Cloud Vision
        </p>
      </header>
      ...
      <footer className="px-6 pb-10 md:px-12 max-w-6xl mx-auto">
        <p className="font-mono text-[11px] text-ink-soft/50">
          รูปภาพถูกส่งไปยัง Google Cloud Vision API เพื่อประมวลผลเท่านั้น ไม่มีการบันทึกถาวรฝั่งเซิร์ฟเวอร์นี้
        </p>
      </footer>
    </div>
  );
